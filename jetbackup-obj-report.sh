...
for LOG in "${QUEUE_DIR}"/*.log; do
    ...
    # FIX: capture +0000 timezone offset so date conversion to Bangkok is correct
    START=$(grep 'Syncing.*homedir' "$LOG" 2>/dev/null | tail -1 | grep -oP '\d{2}/\w+/\d{4} \d{2}:\d{2}:\d{2} \+\d{4}')
    END=$(grep 'Backup Completed' "$LOG" 2>/dev/null | tail -1 | grep -oP '\d{2}/\w+/\d{4} \d{2}:\d{2}:\d{2} \+\d{4}')

    [ -z "$START" ] || [ -z "$END" ] && continue

    # FIX: filter uses Bangkok date (converted from UTC via +0000)
    ...
    # FIX: all display times converted to Bangkok via TZ + correct epoch (which now includes +0000)
